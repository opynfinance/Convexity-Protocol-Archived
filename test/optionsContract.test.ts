...
import {
  ERC20MintableInstance, MockCompoundOracleInstance, OptionsContractInstance, OptionsExchangeInstance,
...
function checkRepo(repo: any,
                   { '0': expectedCollateral, '1': expectedPutsOutstanding, '2': expectedOwner }: { '0': string; '1': string; '2': string; }) {
...
contract('OptionsContract', (accounts) => {
  ...
  describe('#addERC20Collateral()', () => {
    ...
    it('should not be able to add ERC20 collateral to non-ERC20 collateralized options contract', async () => {
      try {
        const repoNum = 1;
        const msgValue = '10000000';
        const result = await optionsContracts[ 0 ].addERC20Collateral(repoNum, '0', {
          from: firstOwnerAddress,
          gas: '100000',
          value: msgValue
        });
  ...
  describe('#exercise', () => {
    describe('during expiry window', () => {
      it('should be able to call exercise', async () => {
        ...
        await optionsContracts[ 0 ].approve(optionsContracts[ 0 ].address, '10000000000000000',
    ...
    describe('after expiry window', () => {
      it('first person should be able to collect their share of collateral', async () => {
        const repoIndex = '1';
        const tx = await optionsContracts[ 0 ].claimCollateral(repoIndex, { from: creatorAddress, gas: '1000000' });
        ...
        expect(tx.logs[ 0 ].args.amtCollateralClaimed.toString()).to.equal('19999700');
      ...
      it('the second person should be able to collect their share of collateral', async () => {
        const repoIndex = '2';
        const tx = await optionsContracts[ 0 ].claimCollateral(repoIndex, { from: firstOwnerAddress, gas: '1000000' });

        // check the calculations on amount of collateral paid out and underlying transferred is correct
        expect(tx.logs[ 0 ].event).to.equal('ClaimedCollateral');
        expect(tx.logs[ 0 ].args.amtCollateralClaimed.toString()).to.equal('9999850');
  ...
  describe('#liquidate()', () => {
    ...
    it('should be able to liquidate if still undercollateralized', async () => {
      ...
      // Try to liquidate the repo
      const tx = await optionsContracts[ 0 ].liquidate(repoIndex, '1000', { from: firstOwnerAddress, gas: '100000' });
